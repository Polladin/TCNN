...
#     return (2 / (1 + math.exp(-u / T)) - 1)
    ...
    return 1 / (1 + math.exp(-u / Eps))


def solve_one_step(X, S, I, Dxy, step_length, N_global, E_global):
    tmp_X = [0] * len(X[:])

    tmp_X[0] = X[0] + 1
    tmp_X[1] =K_s * S + Kf * X[1]  #N_global #
    tmp_X[2] = - alpha * X[3] + I + Kr * X[2] #E_global #
    tmp_X[3] = func_to_calc_sigmoid(tmp_X[1] + tmp_X[2])

#     tmp_X[3] = func_to_calc_sigmoid(S)
